...
class Cell:
    def __init__(self, r, c):
        self.row = r
        self.column = c
        self.walls = [False] * 4  # True -> wall or known to not solve
        self.visited = False
...
while True:
    ...
    move_chosen = None
    for move in filter(lambda m: (not curr_cell.walls[m]) and (not find_destination(curr_cell, m).visited), range(4)):
